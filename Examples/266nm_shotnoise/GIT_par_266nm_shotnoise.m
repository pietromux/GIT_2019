%% README
% This should contain a file description
% This is a simple time-independent Genesis run
% parameters from TESSA266
...
git_par.rxbeam = 73.00e-6;   % Matched beam radius (quadrupole_lattice = 0)
git_par.rybeam = 73.00e-6;   % Matched beam radius
...
git_par.curlen = 20e-6;     % Electron beam pulse length
git_par.ntail = 0;
git_par.bunch = 0.0;
...
p_rad_0=p_rad;
beamtype=1;     % 0 uniform 1 gaussian
write_beamfile;
...
%% untapered undulator simulation
recirculated = 0;     % not recirculated
prebunched = 0;     % not prebunched
git_par.nwig=1;
git_par.awd = 2.82;
git_par.z_stop =git_par.lambda_w0*200;          % Simulation length
GIT_fun_2019;
p_rad_1=power(:,end);
...
%%
fitregion = z>2.5 & z<4.5;
B = polyfit(z(fitregion),log(power(fitregion)), 1);
PowerLgain = 1/B(1) 
fitgain = polyval(B,z);
figure(2)
semilogy(z,exp(fitgain))
hold on
semilogy(z,power)
hold off
title('fit results')
...
for j = 1:size(power,2)
    energy_rad(j) = trapz(zpos./3e8,power(:,j));
end
C = polyfit(z(fitregion),log(energy_rad(fitregion)), 1);
PowerLgain = 1/C(1) 
