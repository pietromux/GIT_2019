...
git_par.writevid=1;                     % write a video file with phasespace evolution
...
git_par.rxbeam = 50.00e-6;   % Matched beam radius
git_par.rybeam = 50.00e-6;   % Matched beam radius
...
git_par.lambda_w0 = 0.025;
git_par.npart = 4096;
git_par.gamma0 = 400;  
git_par.delgamma = git_par.gamma0*0.001;
git_par.emit = 3e-6;
git_par.curpeak = 1300;
git_par.curlen = 0.3e-3;     % Electron beam pulse length
git_par.ntail = 0;
git_par.bunch = 0.7;
git_par.bunchphase = -pi/4; 
%git_par.prad0 = 30e9;
git_par.lambda = 515e-9;
git_par.zrayl = 0.777;
git_par.zwaist = 2;
git_par.nwig = 1;
git_par.ncar = 101;
...
delz = 0.01;
clear z j;
temp = load('-ascii','C:\Users\pietr\Desktop\decelerator\FAST\Polarized Positrons\outres.ppp.txt');
...
Nocibur_periods = [Nocibur_periods Nocibur_periods(end)];
Nocibur_K_init = [Nocibur_K_init Kdephase];
...
Nocibur_periods = [secondsect_periods];
Nocibur_K_init = [secondsect_K];
Nocibur_periods(end+1) = Nocibur_periods(end);
Nocibur_K_init(end+1) = Nocibur_K_init(end);
%Nocibur_periods(end+1) = Nocibur_periods(end);
