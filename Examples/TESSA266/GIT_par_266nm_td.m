...
git_par.curlen = 25e-6;     % Electron beam pulse length
git_par.ntail = 0;
git_par.bunch = 0.0;
...
git_par.nscz = 0;
git_par.nscr = 0;
git_par.pulselength =60e-6;    % Laser pulse length
git_par.beamfile = 1;
git_par.radfile = 1;
...
%% Beam from external beamfile
git_par.nslice=818;
write_beamfile_ext;
filename = strcat(git_par.dirname,'mg.beam.in');
localfile = 'constantenergy.beam'
copyfile(localfile,filename);
current = current';
%curpeakpos = git_par.nslice/2;
[~,curpeakpos] = max(current);
radshift=curpeakpos-100;
write_radfile;
end
zpos=[1:git_par.nslice]*git_par.lambda;timearray=[1:git_par.nslice]*xlamds/3e8;
...
prebunched = 0;                                                            % no input part and rad files 
git_par.period_tapering =0;                                            % No tapering for prebuncher
git_par.psi_0 = 0;                                              
...
git_par.fac0 = 1.0;
git_par.psi_0 =1.15;
git_par.z_stop =.032*29;      % Single undulator simulation.    
